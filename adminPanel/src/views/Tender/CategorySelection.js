...
export default function CustomizedHook(props) {
  const [selectedValues, setSelectedValues] = useState(props.preSelectedCategories || []);  // Initialize with pre-selected categories
  ...
  // When `preSelectedCategories` changes (like when editing), update `selectedValues`
  useEffect(() => {
    if (props.preSelectedCategories) {
      setSelectedValues(props.preSelectedCategories);
    }
          ...
          <Dropdown.Menu className="custom-dropdown-menu">
            ...
            {filteredCategories && filteredCategories.length > 0 ? (
              filteredCategories.map((option) => (
                <Dropdown.Item
                  key={option.id || option.name || Math.random()}
